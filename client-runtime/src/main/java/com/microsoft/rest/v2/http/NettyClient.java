            ...
            HttpResponse httpResponse = new HttpResponse() {
                ...
                public Flux<ByteBuffer> body() {
                    ...
                    Flux<ByteBuffer> javaNioByteBufferFlux = body.map((ByteBuf nettyByteBuf) -> {
                        ...
                        // TODO: Confirm with smaldini
                        // No need to call 'nettyByteBuf.release()' or 'ReferenceCountUtil.release(nettyByteBuf)'.
                        // The upstream 'ByteBufFlux' (impl: reactor.netty.channel.FluxReceive)
                        // takes care of releasing netty buffer once it's subscriber 'onNext' method returns.
                        // Ref: https://github.com/reactor/reactor-netty/blob/ae5dbfac3153c17122ffa47e4c27474906ca4958/src/main/java/reactor/netty/channel/FluxReceive.java#L301
                        //
                        dst.flip();
                        return dst;
                    }).doFinally(s -> {
                        // TODO: Confirm with smaldini
                        // Upon the cancellation, successful completion or errored completion of upstream 'ByteBufFlux' events
                        // need to dispose the Connection (Impl: reactor.netty.http.client.HttpClientOperations).
                        // Ref: https://github.com/reactor/reactor-netty/blob/ae5dbfac3153c17122ffa47e4c27474906ca4958/src/main/java/reactor/netty/channel/ChannelOperations.java#L156
                        //
                        if (!reactorNettyConnection.isDisposed()) {
                            reactorNettyConnection.channel().eventLoop().execute(reactorNettyConnection::dispose);
                        }
                        // TODO: Confirm with smaldini
                        // The reactorNettyConnection needs to be released even though there is no body subscription
                        // hence the rest Response object is Closable, see close() method.
                    });
                    //
